...
import { Loader2, Download, Play, Square, Upload, Film, Save } from 'lucide-react';
...
const ReelsGenerator: React.FC<Props> = ({ onSaved }) => {
  ...
  // Carrega mídia dos exercícios escolhidos (HLS limitado a baixa resolução para não travar)
  useEffect(() => {
    chosen.forEach((ex) => {
        ...
        if (!videosRef.current.has(uid)) {
          ...
          if (video.canPlayType('application/vnd.apple.mpegurl')) {
            ...
            hls.on(Hls.Events.MANIFEST_PARSED, () => {
              // Trava em uma qualidade leve (<= 480p) para reduzir decodificação simultânea
              const capIdx = hls.levels.reduce((best, lvl, i) => {
                if (lvl.height && lvl.height <= 480) {
                  const bestH = hls.levels[best]?.height ?? 0;
                  return lvl.height > bestH ? i : best;
                }
                return best;
              }, 0);
  ...
  useEffect(() => () => {
    ...
    videosRef.current.forEach((v) => { try { v.pause(); v.removeAttribute('src'); v.load(); } catch { /* noop */ } });
    videosRef.current.clear();
    if (bgUrlRef.current) URL.revokeObjectURL(bgUrlRef.current);
  }, []);

  const mediaFor = useCallback((ex: ParsedExercise) => {
    const match = findBestExerciseMatch(ex.exercise, dbExercises);
    const uid = extractStreamVideoId(match?.video_embed);
  ...
  // Loop de render — limitado a 15fps na pré-visualização, 30fps na gravação
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const minGap = recording ? 1000 / 30 : lightPreview ? 1000 / 12 : 1000 / 24;
      if (now - lastFrameRef.current < minGap) return;
      ...
      // Só mantém em reprodução os vídeos visíveis nesta tela
      const activeMedia = new Set<HTMLVideoElement>();
      const items: ReelExerciseItem[] = activePage.map((ex) => {
        const media = mediaFor(ex);
        if (media instanceof HTMLVideoElement) activeMedia.add(media);
        return {
          name: ex.exercise,
          detail: detailFor(ex),
      ...
      videosRef.current.forEach((v) => {
        if (activeMedia.has(v)) { if (v.paused) v.play().catch(() => {}); }
      ...
      <Card className="lg:sticky lg:top-4 h-fit">
        ...
        <CardContent className="space-y-3">
          ...
          <label className="flex items-center gap-2 justify-center text-xs text-muted-foreground cursor-pointer">
            <Checkbox checked={lightPreview} onCheckedChange={(v) => setLightPreview(!!v)} />
            Pré-visualização leve (menos travamentos)
          </label>
        </CardContent>
      </Card>
    </div>
