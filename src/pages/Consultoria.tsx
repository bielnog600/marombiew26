...
const Consultoria: React.FC = () => {
  ...
  const inactiveStudents = useMemo(() => {
    return weeklySummaries.filter(s => {
      ...
      const days = (Date.now() - new Date(s.lastActivity).getTime()) / 86400000;
      return days > 3;
    });
  }, [weeklySummaries]);
                ...
                <>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => { refreshNotifs(); generateBehavioral(); reloadWeekly(); reloadFollowups(); reloadProgression(); }}
                    ...
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground capitalize">{s.attention === 'ok' ? 'Em dia' : 'Atenção'}</p>
                        </div>
                      </div>
                      <Button variant="ghost" size="icon" asChild>
                        <a href={`/alunos/${s.studentId}`}>
