CREATE OR REPLACE FUNCTION public.save_human_first_review(_action text, _exercise_id uuid, _pilot_selection_id text, _classifier_run_id uuid, _reviewer_kind text, _reviewed_metadata jsonb, _field_review_status jsonb, _field_notes jsonb, _evidence jsonb, _expected_version integer, _vocabulary_version text, _server_vocabulary_version text, _change_reason text DEFAULT NULL::text, _changed_fields text[] DEFAULT NULL::text[])
 RETURNS TABLE(id uuid, status text, review_version integer, previous_review_version integer, is_current boolean, diff jsonb, changed_fields text[], reviewed_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_all_keys text[];
  -- F7: server-side constant for this RPC (first human review only)
  c_reviewer_kind constant text := 'human_blinded_v1';
BEGIN
  ...
  END IF;
  -- F7 whitelist: this RPC only serves the first human blinded review.
  IF _reviewer_kind IS DISTINCT FROM c_reviewer_kind THEN
    RAISE EXCEPTION 'invalid_reviewer_kind' USING ERRCODE = '22023';
  END IF;
  ...
  SELECT gt.* INTO v_prev
   ...
   WHERE gt.exercise_id       = _exercise_id
     ...
     AND gt.reviewer_kind     = c_reviewer_kind
  ...
  INSERT INTO public.exercise_metadata_ground_truth (
    ...
    v_uid, c_reviewer_kind, v_now, v_new_version, v_final_status,
