...
DROP FUNCTION IF EXISTS public.save_human_first_review(text,uuid,text,uuid,text,jsonb,jsonb,jsonb,jsonb,integer,text,text,text,text[]);
...
BEGIN
  ...
  IF NOT public.has_role(v_uid,'admin'::public.app_role) THEN RAISE EXCEPTION 'not_authorized' USING ERRCODE='42501'; END IF;
  IF _action NOT IN ('save_draft','finalize','amend_after_final') THEN RAISE EXCEPTION 'invalid_action: %', _action; END IF;
  IF _reviewer_kind IS NULL OR length(_reviewer_kind)=0 THEN RAISE EXCEPTION 'reviewer_kind_required'; END IF;
  IF _vocabulary_version IS DISTINCT FROM _server_vocabulary_version THEN
    RAISE EXCEPTION 'vocabulary_version_mismatch: server=% client=%', _server_vocabulary_version, _vocabulary_version USING ERRCODE='40001';
  END IF;

  SELECT * INTO v_prev FROM public.exercise_metadata_ground_truth
   WHERE exercise_id=_exercise_id AND reviewer_id=v_uid AND reviewer_kind=_reviewer_kind
     AND classifier_run_id=_classifier_run_id AND is_current=true
   FOR UPDATE;

  IF v_prev.id IS NULL THEN
    IF _expected_version <> 0 THEN RAISE EXCEPTION 'version_conflict: server=0 client=%', _expected_version USING ERRCODE='40001'; END IF;
    v_new_version := 1;
  ELSE
    IF v_prev.review_version <> _expected_version THEN
      RAISE EXCEPTION 'version_conflict: server=% client=%', v_prev.review_version, _expected_version USING ERRCODE='40001';
    END IF;
    v_new_version := v_prev.review_version + 1;
    IF v_prev.status = 'human_first_review' THEN
      IF _action = 'save_draft' THEN RAISE EXCEPTION 'cannot_draft_after_finalize' USING ERRCODE='22023'; END IF;
    ...
    IF v_prev.status='human_first_review' AND (v_computed_changed IS NULL OR array_length(v_computed_changed,1) IS NULL) THEN
      RAISE EXCEPTION 'amendment_without_changes' USING ERRCODE='22023';
    END IF;

    UPDATE public.exercise_metadata_ground_truth SET status='superseded', is_current=false, updated_at=v_now WHERE id=v_prev.id;
  END IF;
  ...
  RETURN QUERY SELECT v_new_id, v_final_status, v_new_version, v_prev.review_version, true, v_diff, v_computed_changed, v_now;
END;
$function$;

REVOKE ALL ON FUNCTION public.save_human_first_review(text,uuid,text,uuid,text,jsonb,jsonb,jsonb,jsonb,integer,text,text,text,text[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.save_human_first_review(text,uuid,text,uuid,text,jsonb,jsonb,jsonb,jsonb,integer,text,text,text,text[]) FROM anon;
REVOKE ALL ON FUNCTION public.save_human_first_review(text,uuid,text,uuid,text,jsonb,jsonb,jsonb,jsonb,integer,text,text,text,text[]) FROM service_role;
GRANT EXECUTE ON FUNCTION public.save_human_first_review(text,uuid,text,uuid,text,jsonb,jsonb,jsonb,jsonb,integer,text,text,text,text[]) TO authenticated;
